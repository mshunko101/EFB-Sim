...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(CEFBSimView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(CEFBSimDoc, (CObject*) m_pDocument->GetContainer());
